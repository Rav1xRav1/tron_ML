...
class Brain:
    def __init__(self, net, is_gpu, gamma=0.9, r=0.999, lr=0.01, debug=False):
    ...
    def train(self, states, next_states, action, reward, is_finished):  # ニューラルネットワークを訓練
        ...
        if not is_finished: next_states = torch.from_numpy(next_states).float()  # テンソルに変換
        ...
        self.net.eval()  # 評価モード
        if not is_finished: next_q = self.net(next_states)  # 次の場面をnnに通して値をとる
